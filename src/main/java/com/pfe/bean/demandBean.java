...
private List<Clientscore> listfitre=new ArrayList<>() ;
private HttpSession session =sessionlog.getSession();

    public List<Clientscore> getListfitre() {
        if(demand==null){
         demand=new Demande();
        }if(demand.getIdDemande()!=0){
            System.out.println("id"+demand.getIdDemande());
...
listskils=new ArrayList<>();
  ...
  e=entjpa.findOne(i);
     listedemande.addAll(e.getListdemand().stream().sorted(Comparator.comparingInt(Demande::getExperience)).collect(Collectors.toList()));
        return listedemande;
    }
...
public void trye( ) {
       System.out.println("trye"+demand.getIdDemande());
}
